...
class NpmPackage {
   ...
   *   Event to echo output.
   */
  public static function npmRun(Event $event) {
    static::runNpmCommand('run exec', $event);
  }
  ...
  protected static function findNpmPackages($packageRoot) {
    ...
    $finder = new Finder();
    return $finder->in($packageRoot)->notPath("/core|contrib|vendor|node_modules|bower_components/")->name("package.json");
  }
