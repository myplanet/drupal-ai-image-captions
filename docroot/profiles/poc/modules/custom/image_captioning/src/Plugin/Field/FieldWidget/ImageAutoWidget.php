...
namespace Drupal\image_captioning\Plugin\Field\FieldWidget;
 ...
 * @FieldWidget(
 *   id = "image_image",
 *   label = @Translation("Image Auto Captioning"),
 *   field_types = {
 *     "image"
 *   }
 * )
 */
class ImageAutoWidget extends ImageWidget {
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    $element['#captioning_use_probability'] = $field_settings['captioning_use_probability'];
  ...
  public static function process($element, FormStateInterface $form_state, $form) {
    ...
    if ((bool) $item['fids']) {
      $element['ibm_selector'] = [
        '#type' => 'fieldset',
        '#title' => t('IBM Generate Image caption'),
        '#access' => (bool) $item['fids'] && $element['#alt_field'],
        '#weight' => -12,
      ];
      $element['ibm_selector']['ibm_select_option'] = [
        '#type' => 'select',
        '#access' => (bool) $item['fids'] && $element['#alt_field'],
        '#weight' => -12,
        '#attributes' => [
          'class' => ['ibm-select-option']
        ],
        '#title' => 'Some options',
        '#limit_validation_errors' => [],
        '#options' => [
          0 => t('Click on "Generate Alt" to get suggestions')
        ]
      ];
      $element['ibm_selector']['ibm_button_get'] = [
        '#type' => 'button',
        '#value' => t('Generate Alt'),
        '#access' => (bool) $item['fids'] && $element['#alt_field'],
        '#weight' => -11,
        '#attributes' => [
          'class' => ['ibm-select-button'],
          'data-fid' => !empty($file) ? $file->id() : NULL,
          'data-use-probability' => $element['#captioning_use_probability'],
        ],
        '#suffix' => '<span class="ajax-progress-throbber"><span class="throbber hidden">Loading...</span></span>'
      ];

      $element['#attached']['library'][] = 'image_captioning/image_captioning';
    }
    ...
    $element['#prefix'] = '<div id="ajax-wrapper">';
    $element['#suffix'] = '</div>';
